...
INT CAppGame::Create()
{
	...
	INT		hr = 0;
	INT		spt_type = LC_OBJ_GUI;		// sprite for model coordinate
	...
	MAKE_ARG1(args, (UINT)spt_type);
	if(LC_FAILED(LcDev_CreateSprite(NULL, &m_pSpt, &args)))
		goto End;

	LcVar_WriteP(LC_VAR_SPRITE, m_pSpt);
