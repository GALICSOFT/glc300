...
int CMain::SetupShader()
{
	...
	MAKE_ARG2(args, (UINT)GL_VERTEX_SHADER, (char*)MEDIA_DIR"shader/04sl_vs.glsl");

	hr = LcDev_CreateShaderFromTxtFile(&m_VsShader, &args);
	if(0>hr)
		return -1;

	MAKE_ARG2(args, (UINT)GL_FRAGMENT_SHADER, (char*)MEDIA_DIR"shader/04sl_fs.glsl");
...
int	CMain::Render()
{
	...
	// Linking data to shader
	glUniformMatrix4fv(nUsPrj, 1, 0, (GLfloat*)&m_mtPrj);
	glUniformMatrix4fv(nUsViw, 1, 0, (GLfloat*)&m_mtViw);
	glUniformMatrix4fv(nUsWld, 1, 0, (GLfloat*)&  mtWld);
