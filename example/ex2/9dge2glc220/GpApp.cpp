...
CGpApp::CGpApp()
{
	...
	m_Player.nAlive= PLAYERINITLIFE;
	m_bIsFilre = LC_FALSE;
	...
	m_ShieldTimeBgn = (DOUBLE)LcAux_TimeGetTime();
	m_ShieldExtend = 1.F;
...
INT CGpApp::Create(PLC_ARGS pArg)
{
	...
	LCXVECTOR3	vcPos = LCXVECTOR3(3, 400, 0.0F);
	LCXCOLOR	dCol  = LCXCOLOR(1.0F, 1.0F, 0.0F, 1);


	LOGI("Create Font for system. --------------------\n");
...
INT CGpApp::Cleanup()
{
	...
	LcSys_DestroyWindow();		// Release Window
	LOGI("CGpApp::Cleanup()\n");
					...
					)
{
	...
	*pOut = NULL;
	...
	LcVar_WriteP(LC_VAR_APP_MAIN, pObj);

	if(0 > pObj->Create(pArg))
	{
		LcVar_WriteP(LC_VAR_APP_MAIN, NULL);
		delete pObj;
		return LC_EFAIL;
	}
